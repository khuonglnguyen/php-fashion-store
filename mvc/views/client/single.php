...
<body>
  ...
  ?>
  <nav class="navbar">
    <div class="logo">THUHIEN STORE</div>
  ...
  <div class="title" id="point">Sản phẩm</div>
        ...
        if ($data['star'] > 0) {
          ...
          <div class="product-color">
            ...
            <form action="" method="GET">
              <select name="color" id="color" class="choose-color" onchange="this.form.submit()">
                <?php
                if (!$data['color']) { ?>
                ...
                foreach ($data['colorList'] as $key => $value) {
                  if ($data['color'] != false && $data['color'] == $value['name']) { ?>
          ...
          <div class="product-size">
            <h3>Size: </h3>
            <form action="" method="GET">
              <input type="hidden" value="<?= $_GET['color'] ?>" name="color">
              <select name="size" id="size" class="choose-size" onchange="this.form.submit()">
                <?php
                if (!$data['size']) { ?>
                ...
                if ($data['color']) {
                  foreach ($data['sizeList'] as $key => $value) {
                    if ($data['size'] != false && $data['size'] == $value['name']) { ?>
                      <option selected value="<?= $value['name'] ?>"><?= $value['name'] ?></option>
                    <?php } else { ?>
                      <option value="<?= $value['name'] ?>"><?= $value['name'] ?></option>
                    <?php } ?>
          ...
          if (isset($_GET['color']) && isset($_GET['size'])) { ?>
            <a href="<?= URL_ROOT . '/cart/addItemcart/' .  $data['product']['id'] . '/' . $_GET['color'] . '/' . $_GET['size']  ?>" class="cart-btn">Thêm vào giỏ</a>
          <?php } else { ?>
            <a href="" class="cart-btn">Vui lòng chọn đủ thông tin để thêm vào giỏ</a>
          <?php  }
          ?>
      </div>
    ...
    if ($data['productSuggest']) {
      foreach ($data['productSuggest'] as $key) { ?>
        <div class="card">
          ...
          <p class="qty-card">Kho: ...</p>
          <p class="sold-count">Đã bán: <?= $key['soldCount'] ?></p>
          <p><a href="<?= URL_ROOT . '/product/single/' . $key['id'] ?>"><button>Xem chi tiết</button></a></p>
        </div>
      <?php }
    } else { ?>
