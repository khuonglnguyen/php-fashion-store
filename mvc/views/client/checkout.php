...
<body>
    ...
    if (isset($data['cart']) && count($data['cart']) > 0) { ?>
      ...
      ?>
        <tr>
          ...
          <td><input type="number" min="1" class="qty" name="" id="<?= $value['cartId'] ?>" value="<?= $value['quantity'] ?>" onchange="update(this)"></td>
          <td><?= $value['color'] ?></td>
          <td><?= $value['size'] ?></td>
          <td><?= number_format($value['productPrice'], 0, '', ',') ?>₫</td>
          <td><a href="<?= URL_ROOT . '/cart/removeItemcart/' . (isset($value['cartId']) ? $value['cartId'] : $key) ?>" class="rm-item-cart"><i class="fa fa-trash"></i></a></td>
        </tr>
      <?php }
      ?>
      <tr>
  ...
  <script>
    function update(e) {
      ...
      xhr.open("GET", "http://localhost/thuctap/cart/updateItemcart/" + e.id + "/" + e.value, true);
      xhr.onload = function() {
        if (xhr.readyState === 4) {
          if (xhr.readyState === 4) {
