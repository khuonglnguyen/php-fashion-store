    ...
    if ($data['productList']) {
      foreach ($data['productList'] as $key=>$value) {?>
        <div class="card">
          ...
          <a href="<?= URL_ROOT . '/product/single/' . $value['id'] ?>" style="text-align: center;">
            <h3><?= $value['name'] ?></h3>
          </a>
          <?php
          if ($value['promotionPrice'] < $value['originalPrice']) { ?>
          ...
          <p class="qty-card">Kho: ...
          <p class="sold-count">Đã bán: <?= $value['soldCount'] ?></p>
          <p><a href="<?= URL_ROOT . '/product/single/' . $value['id'] ?>"><button>Xem chi tiết</button></a></p>
        </div>
      <?php }
    } else { ?>
