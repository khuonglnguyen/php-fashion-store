...
class questionModel
{
    ...
    public function getAllAdmin($page = 1, $total = 8)
    {
        ...
        $tmp = ($page - 1) * $total;
        $db = DB::getInstance();
        $sql = "SELECT p.id, p.content, p.reply, u.id as userId, u.fullName, c.name as productName FROM question p JOIN users u ON p.userId = u.id JOIN products c ON p.productId = c.id LIMIT $tmp,$total";
