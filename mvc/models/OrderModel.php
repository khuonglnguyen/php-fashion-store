    ...
    public function add($userId, $total, $percentDiscount = 0)
    {
        ...
        if ($result) {
            ...
            foreach ($resultCart as $key => $value) {
                $s = "INSERT INTO `order_details` (`id`, `orderId`, `productId`, `qty`, `productPrice`, `productName`,`color`,`size`) VALUES (NULL,'" . $newId . "', '" . $value['productId'] . "', '" . $value['quantity'] . "', " . $value['productPrice'] . ", '" . $value['productName'] . "', '" . $value['color'] . "', '" . $value['size'] . "')";
